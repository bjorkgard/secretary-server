...
<template>
    ...
    <div class="isolate px-6 py-24 sm:py-32 lg:px-8">
        ...
        <div class="mx-auto max-w-2xl text-center">
            <h1 class="text-3xl font-black tracking-tight text-gray-900 sm:text-4xl dark:text-slate-300">{{__('page.reports.form.headline')}}: {{report.publisher_name}}</h1>
            <p class="mt-2 text-lg leading-8 text-gray-600 dark:text-slate-300">{{__('page.reports.form.publisherDescription')}}</p>
        </div>
        <div class="mx-auto mt-16 max-w-xl sm:mt-20">
            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                <div class="form-control w-full">
                    ...
                    <select class="select select-bordered" v-model="form.attend">
                        <option disabled :value="null">{{__('common.select')}}</option>
                ...
                <div className="form-control w-full">
                    ...
                    <input id="studies" type="number" class="input input-bordered w-full" v-model="form.studies" :disabled="form.attend !== 'YES'" />
                </div>
                ...
                <div class="form-control w-full">
                    ...
                    <input id="hours" type="number" class="input input-bordered w-full" v-model="form.hours" :disabled="form.attend !== 'YES' || (!form.auxiliary && report.type === 'PUBLISHER')" />
                </div>
            ...
            <div class="mt-10">
                <button type="button" class="w-full btn btn-primary" :class="{ 'opacity-25': form.processing }" :disabled="form.processing || !form.attend" @click="saveReport">{{__('common.send')}}</button>
            </div>
        </div>
    </div>
</template>
